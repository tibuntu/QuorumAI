...
### M6 — Review Depth & Polish  ✅ shipped (all on `main`)
...
- **P3 · Quorum / N-approver thresholds** ✅ shipped (on `main`, executed in a separate session) — owners can require N approvals (1–10) before `APPROVED`. No schema migration (the `requiredApprovals` column + `computeDocumentState` already existed; the field was just never set). Pure `lib/quorum.ts` (`parseRequiredApprovals` 1–10, `approvalCount`); set at create (web `POST /api/documents` + machine `POST /api/plans` + `NewDocumentForm`); edited via dedicated owner-only routes `PATCH /api/documents/[id]/settings` + `/api/plans/[id]/settings`; `setRequiredApprovals` service recomputes state via a recompute helper **extracted from + shared with** `submitReview` (lower threshold → APPROVED, raise → OPEN, REQUEST_CHANGES unaffected); "N of M approvals" progress + owner threshold control in `DocumentView`; `requiredApprovals`+`approvals` added to the `consolidateFeedback` contract (additive, no schemaVersion bump). Full gate green (unit+e2e+lint+tsc+build). Plan: `plans/2026-06-10-quorum-ai-m6-p3-quorum-thresholds.md`; design: `specs/2026-06-10-quorum-ai-m6-p3-quorum-thresholds-design.md`.

Dropped from the backlog entirely in M6: dedicated Slack/Teams formatters; git export.

## Git state
- `main`: M1–M5 + **all of M6 (P1+P2+P3)** landed locally (each fast-forwarded in from its worktree branch, now removed). **`main` is ahead of `origin` — not yet pushed.** **P2 added 2 migrations** (P3 added none) — run `pnpm db:deploy` (+ `pnpm prisma generate`) on the main checkout before `pnpm dev`/tests (DB is per-checkout).
...
## Next action
**M6 is complete** — all three phases (P1 UI polish, P2 granular notification prefs, P3 quorum thresholds) shipped on local `main`. No active phase. Next is a milestone-level decision: **push `main` to `origin`** (it holds M1–M6, unpushed), and/or scope **M7** from the deferred list (Postgres & multi-instance · teams/org & multi-tenancy · admin/moderator roles · enforced-SSO/SCIM · soft-delete/trash · version checkpointing/compaction · multi-hunk suggestion patches). Standing reminder: M4/P5 was a **breaking** env rename — deploys must set `BASE_URL`/`AUTH_SECRET` before upgrading.
